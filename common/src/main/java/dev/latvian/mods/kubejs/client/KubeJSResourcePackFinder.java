...
public class KubeJSResourcePackFinder implements RepositorySource {
	...
	public void loadPacks(Consumer<Pack> nameToPackMap, Pack.PackConstructor packInfoFactory) {
		if (Files.notExists(KubeJSPaths.ASSETS)) {
			...
			try (var in = KubeJS.class.getResourceAsStream("/data/kubejs/example_block_texture.png");
				 var out = Files.newOutputStream(KubeJSPaths.ASSETS.resolve("kubejs/textures/block/example_block.png"))) {
				out.write(IOUtils.toByteArray(in));
			} catch (Exception ex) {
				ex.printStackTrace();
			}

			try (var in = KubeJS.class.getResourceAsStream("/data/kubejs/example_item_texture.png");
				 var out = Files.newOutputStream(KubeJSPaths.ASSETS.resolve("kubejs/textures/item/example_item.png"))) {
				out.write(IOUtils.toByteArray(in));
			} catch (Exception ex) {
				ex.printStackTrace();
			}
