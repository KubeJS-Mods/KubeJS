...
public class KubeJSEntityEventHandler {
	...
	private static EventResult checkSpawn(LivingEntity entity, LevelAccessor la, double x, double y, double z, MobSpawnType type, BaseSpawner spawner) {
		if (la instanceof ServerLevel level && UtilsJS.staticServer != null && EntityEvents.CHECK_SPAWN.post(getTypeId(entity.getType()), new CheckLivingEntitySpawnEventJS(entity, level, x, y, z, type))) {
	...
	private static EventResult entitySpawned(Entity entity, Level level) {
		if (entity != null && level instanceof ServerLevel && UtilsJS.staticServer != null && EntityEvents.SPAWNED.post(getTypeId(entity.getType()), new EntitySpawnedEventJS(entity, level))) {
			return EventResult.interruptFalse();
		}
