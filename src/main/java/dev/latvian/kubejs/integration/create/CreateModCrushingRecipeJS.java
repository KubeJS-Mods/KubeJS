 ...
 */
public class CreateModCrushingRecipeJS extends RecipeJS
{
	public static final RecipeTypeJS TYPE = new RecipeTypeJS(new ResourceLocation("create:crushing"))
	{
		@Override
		public RecipeJS create(ListJS args)
		{
			if (args.size() < 2)
			{
				return new RecipeErrorJS("Create crushing recipe requires at least 2 arguments - result array and ingredient!");
			}

			CreateModCrushingRecipeJS recipe = new CreateModCrushingRecipeJS();

			ListJS results = ListJS.orSelf(args.get(0));

			if (results.isEmpty())
			{
				return new RecipeErrorJS("Create crushing recipe results can't be empty!");
			}
			...
			for (Object o : results)
			{
				...
				if (stack.isEmpty())
				{
					return new RecipeErrorJS("Create crushing recipe result " + o + " is not a valid item!");
				}
				else
				{
			...
			{
				return new RecipeErrorJS("Create crushing recipe ingredient " + args.get(1) + " is not a valid ingredient!");
			}
		...
		public RecipeJS create(JsonObject json)
		{
			...
			for (JsonElement e : json.get("results").getAsJsonArray())
			{
				...
				if (stack.isEmpty())
				{
					return new RecipeErrorJS("Create crushing recipe result " + e + " is not a valid item!");
				}
				else
				{
					recipe.results.add(stack);
			...
			if (recipe.ingredient.isEmpty())
			{
				return new RecipeErrorJS("Create crushing recipe ingredient " + in + " is not a valid ingredient!");
			}
	...
	public CreateModCrushingRecipeJS time(int t)
	{
