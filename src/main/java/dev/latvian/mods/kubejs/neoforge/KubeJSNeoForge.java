...
@Mod(KubeJS.MOD_ID)
public class KubeJSNeoForge {

	private static final ThreadLocal<IEventBus> BUS = new ThreadLocal<>();

	public KubeJSNeoForge(IEventBus bus) throws Throwable {
		bus.addListener(EventPriority.LOW, KubeJSNeoForge::loadComplete);
		bus.addListener(EventPriority.LOW, KubeJSNeoForge::initRegistries);
		bus.addListener(EventPriority.LOW, KubeJSNeoForge::commonSetup);
		bus.addListener(EventPriority.LOW, KubeJSNeoForge::creativeTab);
		...
		NeoForge.EVENT_BUS.addListener(KubeJSNeoForge::itemDestroyed);
		NeoForge.EVENT_BUS.addListener(EventPriority.LOWEST, KubeJSNeoForge::livingDrops);
