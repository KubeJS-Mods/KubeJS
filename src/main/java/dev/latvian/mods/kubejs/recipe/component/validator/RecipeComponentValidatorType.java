...
public record RecipeComponentValidatorType<T extends RecipeComponentValidator>(ResourceLocation id, MapCodec<T> codec) {
	public static final Lazy<Map<ResourceLocation, RecipeComponentValidatorType<?>>> TYPES = Lazy.map(map -> KubeJSPlugins.forEachPlugin(type -> map.put(type.id, type), KubeJSPlugin::registerRecipeComponentValidatorTypes));
